...
exports.wrapError = exports.wrapClient = void 0;
...
function wrapError(error, path) {
    ...
    if (isNotFoundError(error)) {
        return new errors_1.NotFoundError(path);
    }
    if (isForbiddenError(error)) {
        return new errors_1.ForbiddenError(path);
    }
    if (isConflictError(error)) {
        return new errors_1.ConflictError(path);
    }
    return new errors_1.NextCloudServerException('A WebDav Error occured', error);
}
